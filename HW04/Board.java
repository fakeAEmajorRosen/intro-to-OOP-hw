...
public class Board {
    ...
    public boolean checkLocation(Location location) {
        return (location.getRow() >= 0 && location.getRow() <= 2
            && location.getColumn() >= 0 && location.getColumn() <= 2)
            && (this.board[location.getRow()][location.getColumn()] == ' ');
    }

    public boolean placeSign(Location location, char letter) {
        if (this.checkLocation(location)) {
            this.board[location.getRow()][location.getColumn()] = letter;
            return true;
        } else {
            return false;
        }
